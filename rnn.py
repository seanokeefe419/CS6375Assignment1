...
class RNN(nn.Module):
    ...
    def forward(self, inputs):
        ...
        _, hidden = (inputs, torch.randn(self.numOfLayer, 1, self.h))
        
        # [to fill] obtain output layer representations
        rnn1, _ = self.rnn(inputs, hidden)
        ol = self.W(rnn1)
        # [to fill] sum over output 
        # print(ol.size())
        # output_sum = torch.sum(ol, 0, keepdim = True)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--test_data", required = True, default = "to fill", help = "path to test data")
